 ...
 public   class Setting
    {
     ...
     internal static void log(string msg, int maxValue, ref int value)
     {
         lock (_object) {
             if (value > maxValue)
             {
                 System.IO.File.AppendAllText(Setting.logFileName + "\\logExe" + DateTime.Now.ToString("yyyy-MM-dd") + ".txt", DateTime.Now.ToString() + " : " + msg + "error falt:" + value + "\r\n");
                 value = 0;
             }
             else
             {
     ...
     internal static bool logStatusSystem(string msg, int maxValue,  int value)
     {
         lock (_object1) {
             if (value > maxValue)
             {
                 System.IO.File.AppendAllText(Setting.logFileName + "\\logStatusSystem" + DateTime.Now.ToString("yyyy-MM-dd") + ".txt", DateTime.Now.ToString() + " : " + msg + "error falt:" + value + "\r\n");
                 return true;
             }
             else
             {
     ...
     internal static int getHoursInDay()
     {
        ...
        public static string you_cant_add_content = " شما به علت رفتارهای نامتعارف ،حق انتشار محتوا را ندارید";
        public static string you_cant_use_robot = " شما به علت رفتارهای نامتعارف ،حق استفاده از روبات را ندارید";
