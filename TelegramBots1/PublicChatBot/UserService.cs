...
namespace PublicChatBot
{
    public class UserService : BaseService
    {
        ...
        private void _addUser(Message message_req, User user)
        {
            ...
            dbContext.DbWriteble.Users.Add(user);
            dbContext.DbWriteble.SaveChanges();
        }
        ...
        private void _addBridge_UserBot(string botUserName, User user, Bridge_BotUser b_botUser, byte role, string userShear)
        {
            ...
            b_botUser.role = role;
            content _content = dbContext.DbWriteble.contents.FirstOrDefault(c => c.fk_bot == botModel.botName
            ...
            dbContext.DbWriteble.Bridge_BotUser.Add(b_botUser);
            int res = dbContext.DbWriteble.SaveChanges();
        ...
        {
            return dbContext.DbWriteble.Users.FirstOrDefault(u => u.id == user_id);
        }

        private Bridge_BotUser _getB_botUser(string botUserName, int user_id)
        {
            return dbContext.DbWriteble.Bridge_BotUser.FirstOrDefault(b => b.fk_bot == botUserName && b.fk_user == user_id);
        }
        /*
        ...
        {
            return dbContext.DbWriteble.Bridge_BotUser.Where(b => b.fk_bot == botUserName);
        }

        internal void getTopRate(int chatId)
        {
            string message = "";
            IQueryable<User> iQuerybleUser = dbContext.DbWriteble.Users.OrderByDescending(u => u.contents
        ...
        {
            int rate = 0;
            var iEContentUser = dbContext.DbWriteble.contents.Where(b => b.fk_bot == botModel.botName && b.fk_user == chatId);
            if (iEContentUser.Count() > 0)
            {
                rate = iEContentUser.Sum(b => b.ratePlus + b.rateMinus);
            }
            var bridge_BotUser = dbContext.DbWriteble.Bridge_BotUser.FirstOrDefault(b => b.fk_bot == botModel.botName && b.fk_user == chatId);
        ...
        {
            int countUser = dbContext.DbWriteble.Bridge_BotUser.Count(u => u.fk_bot == botModel.botName);
        ...
        {
            return dbContext.DbWriteble.Bridge_BotUser.Count(b => b.fk_bot == botModel.botName);
        }
